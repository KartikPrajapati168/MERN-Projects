...
const BuyerDashboard = () => {
  ...
  // Messages (two-pane conversation UI, mirrors Advocare's Messages & Updates page)
  ...
  const pendingDeals = deals.filter(d => d.buyerId === user?._id && d.status === 'requested').length;
  const acceptedDeals = deals.filter(d => d.buyerId === user?._id && d.status === 'accepted').length;
  const completedDeals = deals.filter(d => d.buyerId === user?._id && d.status === 'completed').length;
  const unreadCount = messages.filter(m => m.is_read === false).length;

  // ----- Dashboard donut chart (deal status breakdown) — mirrors Advocare's
  // "Case Status Overview" chart on the Client Portal dashboard -----
  useEffect(() => {
  ...
  // const getAITextMatches = () => {
  //   const activeListings = listings.filter(l => l.status === 'active');
  //   if (!aiQuery.trim()) return getAIMatches();
  //   const q = aiQuery.toLowerCase();
  //   const words = q.split(/\s+/).filter(w => w.length > 2);
  //   const scored = activeListings.map(listing => {
  //     const text = (listing.material + ' ' + (listing.description || '')).toLowerCase();
  //     const hits = words.filter(w => text.includes(w)).length;
  //     const score = Math.min(100, Math.round((hits / Math.max(1, words.length)) * 100) + (text.includes(q) ? 20 : 0));
  ...
  // const aiMatches = findMaterialMode === 'ai' ? getAITextMatches() : [];
  ...
  // ----- Messages (best-effort send; falls back to local echo if no endpoint) -----
      ...
      case 'dashboard':
        return (
          <div className="bd-dashboard">
            ...
            {/* Two-column: chart + recent activity, mirrors Advocare's dashboard-content grid */}
            ...
            {/* Requirements data table, mirrors Advocare's "My Cases" table on Dashboard */}
            <div className="bd-content-card">
              <div className="bd-card-title">
            ...
            {findMaterialMode === 'ai' && (
              <div className="bd-content-card">
                ...
                <div className="bd-ai-box">
                  <p><i className="fas fa-info-circle me-2" style={{ color: '#a78bfa' }}></i>
                    AI analyzes your requirements (or a quick description) and recommends the best <strong>active listings</strong> based on material, quantity, price and location fit.
                  </p>
                  <textarea rows="3" placeholder="Optional: describe what you're looking for (e.g., 500kg clean PET plastic bottles near Ahmedabad)" value={aiQuery} onChange={(e) => setAiQuery(e.target.value)} />
                  <small style={{ color: '#6b7280' }}>Leave this blank to see matches generated automatically from your open requirements.</small>
                </div>

                {aiMatches.length === 0 ? (
                  <div className="bd-empty-state">
                    <div className="bd-empty-icon">🤖</div>
                    <p>No AI recommendations yet. Create a requirement or describe what you need above.</p>
                    ...
                    {aiMatches.slice(0, 12).map((item, idx) => {
                      ...
                      return (
                        <div key={item._id + idx} className="bd-material-card bd-ai-card" style={{ borderColor: bc }}>
                          ...
                          <p className="bd-material-card-sub"><i className="fas fa-map-marker-alt me-1"></i>{item.location} | {item.quantity}kg</p>
                          <div className="bd-material-card-actions">
                ...
                {!selectedConversation ? (
                  ...
                  <>
                    ...
                    <div className="bd-chat-body">
                      {messages.filter(m => m.senderName === selectedConversation).length === 0 ? (
                        <div className="bd-chat-placeholder" style={{ margin: 'auto' }}>
                          <i className="fas fa-comment-dots"></i>
                          <p>No messages yet. Start the conversation!</p>
                        </div>
                      ) : (
                        messages.filter(m => m.senderName === selectedConversation || m.senderName === user?.name).map((m, idx) => (
    ...
    <div className="bd-dashboard-wrapper">
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        /* ----- Messages: two-pane conversation UI (Advocare-style) ----- */
